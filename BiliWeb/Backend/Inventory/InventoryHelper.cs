...
namespace BiliWeb.Backend
{
    public static class InventoryHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static IEnumerable<SelectListItem> ToSelectListItems(this IEnumerable<InventoryModel> dataSet, string selectedId)
        {
            return
                dataSet.OrderBy(m => m.ID)
                      .Select(m =>
                          new SelectListItem
                          {
                              ...
                              Text = m.TestStripStock.ToString()
                          });
        }

        /// <summary>
        /// Converts the ID for a record, to a string for the item.
        /// For Inventory converts id 234-asdf-234-sdf to "Bellevue"
        /// </summary>
        ...
        {
            var data = DataSourceBackend.Instance.InventoryBackend.Read(id);
            if (data == null)
            {
                return string.Empty;
            }
            ...
            return data.TestStripStock.ToString();
        }
    }
}