...
{
    /// <summary>
    /// Define an interface which contains the methods for the Clinic repository.
    /// All CRUDi aspects
    /// 
    /// </summary>
    public interface IClinicRepository
    {
        ...
        ClinicModel Create(ClinicModel data, DataSourceEnum dataSourceEnum = DataSourceEnum.Unknown);

        // Returns the record of the matching ID or null if not found
        ClinicModel Read(String id);

        // Update the record passed in,  the ID and Date are not modified
        ClinicModel Update(ClinicModel data);
